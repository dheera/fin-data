#!/usr/bin/env python3
"""
Download ETF Global taxonomies (classification metadata) from Massive API.
https://massive.com/docs/rest/partners/etf-global/taxonomies

Requires the "ETF Global Taxonomies" add-on on the Massive/Polygon key.
History: 2021-11-01 .. present, daily snapshots, ~3000+ ETFs.
Key fields: rebalance_frequency, reconstitution_frequency, weighting_methodology,
selection_methodology, leverage_style, levered_amount, leverage_reset,
primary_benchmark, asset_class, category, focus, issuer, inception_date,
holdings_disclosure_frequency.

Usage:
  python download_etfglobal_taxonomies.py --smoke        # verify entitlement (1 page)
  python download_etfglobal_taxonomies.py                # full pull, chunked by month, resumable
  python download_etfglobal_taxonomies.py --workers 8

Output:
  etf_global/taxonomies/{YYYY-MM}.parquet   (chunk cache, safe to re-run)
  etf_global/taxonomies.parquet             (merged history)
  etf_global/taxonomies_latest.parquet      (latest snapshot per ETF)
"""
import argparse
...
ENDPOINT = "/etf-global/v1/taxonomies"
PAGE_LIMIT = 5000
SLEEP_SEC = 0.1
FIRST_MONTH = "2021-11"
...
def fetch_json(session: requests.Session, url: str, api_key: str, params: dict | None = None,
    ...
    for i in range(retries):
        ...
        if r.status_code in (402, 403):
            raise SystemExit(
                f"HTTP {r.status_code} NOT AUTHORIZED — the ETF Global Taxonomies add-on is "
...
def main():
    ...
    out_dir = root / "etf_global" / "taxonomies"
    ...
    if not args.no_merge:
        ...
        merged = root / "etf_global" / "taxonomies.parquet"
        ...
        latest = (df.sort_values("effective_date")
                    .drop_duplicates("composite_ticker", keep="last")
                    .reset_index(drop=True))
        latest_path = root / "etf_global" / "taxonomies_latest.parquet"
        latest.to_parquet(latest_path, index=False)
        print(f"LATEST: {len(latest)} ETFs -> {latest_path}")
