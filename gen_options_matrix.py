...
def main():
    parser = argparse.ArgumentParser(description="Aggregate minute option data into call/put volumes and compute vwas.")
    ...
    for file_path in file_paths:
        ...
        # --------------------------------------------------
        # 4) Compute volume-weighted average strike (vwas)
        # --------------------------------------------------
        # Group by (window_start, underlying) across both types
        vwas_df = df.groupby(['window_start', 'underlying']).apply(
            lambda g: (g['volume'] * g['strike']).sum() / g['volume'].sum() if g['volume'].sum() > 0 else 0
        ).reset_index(name='vwas')

        # Pivot so each underlying gets its own column for vwas
        vwas_pivot = vwas_df.pivot(index='window_start', columns='underlying', values='vwas')
        # Ensure all top underlyings are present
        vwas_pivot = vwas_pivot.reindex(columns=top_und, fill_value=0)
        # Create a MultiIndex similar to the pivoted volumes with level-1 label 'vwas'
        vwas_pivot.columns = pd.MultiIndex.from_product([vwas_pivot.columns, ['vwas']])

        # --------------------------------------------------
        # 5) Ensure All NYSE Trading Minutes Are Present
        # --------------------------------------------------
        ...
        # Reindex pivoted volumes and vwas to ensure every minute is present, filling missing values with 0
        pivoted = pivoted.reindex(full_index, fill_value=0)
        vwas_pivot = vwas_pivot.reindex(full_index, fill_value=0)

        # Concatenate the volume and vwas dataframes along the columns
        final_df = pd.concat([pivoted, vwas_pivot], axis=1)

        # Convert volume columns to int32 (vwas remains float)
        final_df = final_df.astype({col: 'int32' for col in final_df.columns if col[1] in ['C_volume', 'P_volume']})

        # --------------------------------------------------
        # 6) Write the DataFrame to Parquet
        # --------------------------------------------------
